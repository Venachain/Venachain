...
const (
	clientIdentifier = "venachain" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the venachain-go command line interface")
...
func geth(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) > 0 {
		return fmt.Errorf("invalid command: %q, for more help:\n        venachain -h", args[0])
	}
