...
[Venachain快速搭链教程](https://git-c.i.wxblockchain.com/Venachain/doc/Venachain_WIKI/blob/v0.9.0/zh-cn/basics/Installation/%5BChinese-Simplified%5D-%E5%BF%AB%E9%80%9F%E9%83%A8%E7%BD%B2.md)
...
[Venachain合约指导文档](https://git-c.i.wxblockchain.com/Venachain/doc/Venachain_WIKI/blob/v0.9.0/zh-cn/WASMContract/%5BChinese-Simplified%5D-%E5%90%88%E7%BA%A6%E6%95%99%E7%A8%8B.md)
...
[change_log文档](https://git-c.i.wxblockchain.com/Venachain/src/node/Venachain-Go/blob/develop/CHANGELOG.md)
EOF
}
...
function env() {
    if [[ -d ${Venachain_Project_name} ]]; then
        ...
        git clone --recursive https://git-c.i.wxblockchain.com/Venachain/src/node/Venachain-Go.git
    fi

    if [[ -d ${Java_Project_name} ]]; then
        echo "${Java_Project_name} already exists"
    else
        git clone --recursive https://git-c.i.wxblockchain.com/Venachain/src/node/java-sdk.git
    fi
