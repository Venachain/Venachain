...
type Ethereum struct {
	...
	// DB interfaces
	chainDb venadb.Database // Block chain database
...
func CreateDB(ctx *node.ServiceContext, config *Config, name string) (venadb.Database, error) {
	db, err := ctx.OpenDatabase(name, config.DatabaseCache, config.DatabaseHandles)
	if err != nil {
		return nil, err
	}
	if db, ok := db.(*venadb.LDBDatabase); ok {
...
// create extended database
func CreateExtDB(ctx *node.ServiceContext, config *Config, name string) (venadb.Database, error) {
	db, err := ctx.OpenDatabase(name, config.DatabaseCache, config.DatabaseHandles)
	if err != nil {
		return nil, err
	}
...
func CreateConsensusEngine(ctx *node.ServiceContext, chainConfig *params.ChainConfig, db venadb.Database) consensus.Engine {
...
func (s *Ethereum) ExtendedDb() venadb.Database        { return s.extDb }
func (s *Ethereum) AccountManager() *accounts.Manager  { return s.accountManager }
func (s *Ethereum) BlockChain() *core.BlockChain       { return s.blockchain }
func (s *Ethereum) TxPool() *core.TxPool               { return s.txPool }
func (s *Ethereum) EventMux() *event.TypeMux           { return s.eventMux }
func (s *Ethereum) Engine() consensus.Engine           { return s.engine }
func (s *Ethereum) ChainDb() venadb.Database           { return s.chainDb }
