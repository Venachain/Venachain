...
type BloomIndexer struct {
	...
	db      venadb.Database      // database instance to write index data and metadata into
...
func NewBloomIndexer(db venadb.Database, size, confirms uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := venadb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
