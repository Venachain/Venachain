...
func GetChtRoot(db venadb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
// Note that sectionIdx is specified according to LES/1 CHT section size.
func StoreChtRoot(db venadb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    venadb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db venadb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	trieTable := venadb.NewTable(db, ChtTablePrefix)
	...
	return core.NewChainIndexer(db, venadb.NewTable(db, "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
func GetBloomTrieRoot(db venadb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db venadb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db venadb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	trieTable := venadb.NewTable(db, BloomTrieTablePrefix)
	...
	return core.NewChainIndexer(db, venadb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}
