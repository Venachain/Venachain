...
function updateToConsensusNode() {
    ...
    ${BIN_PATH}/platonecli node update "${NODE_ID}" --type "${NODE_TYPE}" --keyfile "${KEYFILE}" --url "${FIRSTNODE_IP_ADDR}:${FIRSTNODE_RPC_PORT}" <"${PHRASE}"
    timer=0
    while [ ${timer} -lt 10 ]; do
        res_update_node=$(${BIN_PATH}/platonecli node query --type ${NODE_TYPE} --name ${NODE_ID} --url "${FIRSTNODE_IP_ADDR}:${FIRSTNODE_RPC_PORT}") >/dev/null 2>&1
