...
function addNode() {
    if [ ! -f "${PROJECT_CONF_PATH}/logs/deploy_log.txt" ] || [[ $(grep $(echo $0 | sed -e 's/\(.*\)\/\(.*\).sh/\2/g') "${PROJECT_CONF_PATH}/logs/deploy_log.txt" | grep "node-${NODE_ID}" | grep "Add node") == "" ]]; then
        ...
        if [[ "${FIRSTNODE_ID}" == "${NODE_ID}" ]]; then
            ...
            res_add_node=$(xcmd "${USER_NAME}@${IP_ADDR}" "${DEPLOY_PATH}/bin/platonecli node query --name ${NODE_ID} --url ${IP_ADDR}:${RPC_PORT}") >/dev/null 2>&1
        else
            xcmd "${FIRSTNODE_USERNAME}@${FIRSTNODE_IP_ADDR}" "${FIRSTNODE_DEPLOY_PATH}/scripts/add-node.sh -n ${NODE_ID}" 
            res_add_node=$(xcmd "${USER_NAME}@${IP_ADDR}" "${DEPLOY_PATH}/bin/platonecli node query --name ${NODE_ID} --url ${FIRSTNODE_IP_ADDR}:${FIRSTNODE_RPC_PORT}") >/dev/null 2>&1
        fi
...
function updateNodeToConsensus() {
    if [ ! -f "${PROJECT_CONF_PATH}/logs/deploy_log.txt" ] || [[ $(grep $(echo $0 | sed -e 's/\(.*\)\/\(.*\).sh/\2/g') "${PROJECT_CONF_PATH}/logs/deploy_log.txt" | grep "node-${NODE_ID}" | grep "Update node") == "" ]]; then
        ...
        if [[ "${FIRSTNODE_ID}" == "${NODE_ID}" ]]; then
            ...
            res_update_node=$(xcmd "${USER_NAME}@${IP_ADDR}" "${DEPLOY_PATH}/bin/platonecli node query --type consensus --name ${NODE_ID} --url ${IP_ADDR}:${RPC_PORT}") >/dev/null 2>&1
        else
            xcmd "${FIRSTNODE_USERNAME}@${FIRSTNODE_IP_ADDR}" "${FIRSTNODE_DEPLOY_PATH}/scripts/update_to_consensus_node.sh -n ${NODE_ID}" 
            res_update_node=$(xcmd "${USER_NAME}@${IP_ADDR}" "${DEPLOY_PATH}/bin/platonecli node query --type consensus --name ${NODE_ID} --url ${FIRSTNODE_IP_ADDR}:${FIRSTNODE_RPC_PORT}") >/dev/null 2>&1
        fi
