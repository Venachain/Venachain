...
var (
	...
	// errInvalidMixDigest is returned if a block's mix digest is not Iris digest.
	errInvalidMixDigest = errors.New("invalid Iris mix digest")
...
func (sb *backend) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if _, err := types.ExtractIrisExtra(header); err != nil {
...
func (sb *backend) verifyCommittedSeals(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	extra, err := types.ExtractIrisExtra(header)
	if err != nil {
		return err
	}
	...
	validSeal := 0
	proposalSeal := irisCore.PrepareCommittedSeal(header.Hash())
	// 1. Get committed seals from current header
	for _, seal := range extra.CommittedSeal {
		// 2. Get the original address by seal and parent block hash
		addr, err := iris.GetSignatureAddress(proposalSeal, seal)
		if err != nil {
...
func (sb *backend) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	header.MixDigest = types.IrisDigest
...
func (sb *backend) Seal(chain consensus.ChainReader, block *types.Block, sealResultCh chan<- *types.Block, stop <-chan struct{}) (*types.Block, error) {
	...
	if snap.ValSet.Size() == 1 {
		// post block into Iris engine
		go sb.EventMux().Post(iris.SingleCommittedEvent{
			Proposal: block,
		})
	} else {
		// post block into Iris engine
		go sb.EventMux().Post(iris.RequestEvent{
			Proposal: block,
		})
	}
...
func (sb *backend) APIs(chain consensus.ChainReader) []rpc.API {
	return []rpc.API{{
		Namespace: "iris",
		Version:   "1.0",
		Service:   &API{chain: chain, iris: sb},
		Public:    true,
	}}
}

// Start implements consensus.Iris.Start
func (sb *backend) Start(chain consensus.ChainReader, currentBlock func() *types.Block) error {
	sb.coreMu.Lock()
	defer sb.coreMu.Unlock()
	if sb.coreStarted {
		return iris.ErrStartedEngine
	}
...
// Stop implements consensus.Iris.Stop
func (sb *backend) Stop() error {
	sb.coreMu.Lock()
	defer sb.coreMu.Unlock()
	if !sb.coreStarted {
		return iris.ErrStoppedEngine
	}
...
// FIXME: Need to update this for Iris
// sigHash returns the hash which is used as input for the Iris
...
func ecrecover(header *types.Header) (common.Address, error) {
	...
	istanbulExtra, err := types.ExtractIrisExtra(header)
	if err != nil {
		return common.Address{}, err
	}

	addr, err := iris.GetSignatureAddress(sigHash(header).Bytes(), istanbulExtra.Seal)
	if err != nil {
		return addr, err
	}
...
func recoverPubkey(header *types.Header) (ecdsa.PublicKey, error) {
	...
	pubkey, err := iris.GetSignaturePubkey(sigHash(header).Bytes(), istanbulExtra.Seal)
	if err != nil {
		return *pubkey, err
	}
...
func prepareExtra(header *types.Header, vals []common.Address) ([]byte, error) {
	...
	// compensate the lack bytes if header.Extra is not enough IrisExtraVanity bytes.
	if len(header.Extra) < types.IrisExtraVanity {
		header.Extra = append(header.Extra, bytes.Repeat([]byte{0x00}, types.IrisExtraVanity-len(header.Extra))...)
	}
	buf.Write(header.Extra[:types.IrisExtraVanity])
...
func writeSeal(h *types.Header, seal []byte) error {
	...
	istanbulExtra, err := types.ExtractIrisExtra(h)
	if err != nil {
		return err
	}
	...
	h.Extra = append(h.Extra[:types.IrisExtraVanity], payload...)
	return nil
}
