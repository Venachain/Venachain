...
type backend struct {
	...
	// the channels for iris engine notifications
...
// Address implements iris.Backend.Address
func (sb *backend) Address() common.Address {
	return sb.address
}

// Validators implements iris.Backend.Validators
func (sb *backend) Validators(proposal iris.Proposal) iris.ValidatorSet {
	return sb.getValidators(proposal.Number().Uint64(), proposal.Hash())
}

// Broadcast implements iris.Backend.Broadcast
func (sb *backend) Broadcast(valSet iris.ValidatorSet, payload []byte) error {
	// send to others
	sb.Gossip(valSet, payload)
	// send to self
	msg := iris.MessageEvent{
		Payload: payload,
	}
...
// Broadcast implements iris.Backend.Gossip
func (sb *backend) Gossip(valSet iris.ValidatorSet, payload []byte) error {
	hash := iris.RLPHash(payload)
...
// Commit implements iris.Backend.Commit
func (sb *backend) Commit(proposal iris.Proposal, seals [][]byte) error {
	...
	if !isEmpty || isProduceEmptyBlock {
		//post commit event
		sb.EventMux().Post(iris.CommittedEvent{
...
// EventMux implements iris.Backend.EventMux
...
// Verify implements iris.Backend.Verify
func (sb *backend) Verify(proposal iris.Proposal, isProposer bool) (time.Duration, error) {
	// Check if the proposal is a valid block
	block := &types.Block{}
	block, ok := proposal.(*types.Block)
	if !ok {
...
// Sign implements iris.Backend.Sign
func (sb *backend) Sign(data []byte) ([]byte, error) {
	hashData := crypto.Keccak256([]byte(data))
	return crypto.Sign(hashData, sb.privateKey)
}

// CheckSignature implements iris.Backend.CheckSignature
func (sb *backend) CheckSignature(data []byte, address common.Address, sig []byte) error {
	signer, err := iris.GetSignatureAddress(data, sig)
	if err != nil {
...
// HasPropsal implements iris.Backend.HashBlock
func (sb *backend) HasPropsal(hash common.Hash, number *big.Int) bool {
	return sb.chain.GetHeader(hash, number.Uint64()) != nil
}

// GetProposer implements iris.Backend.GetProposer
...
// ParentValidators implements iris.Backend.GetParentValidators
func (sb *backend) ParentValidators(proposal iris.Proposal) iris.ValidatorSet {
...
func (sb *backend) getValidators(number uint64, hash common.Hash) iris.ValidatorSet {
...
func (sb *backend) LastProposal() (iris.Proposal, common.Address) {
