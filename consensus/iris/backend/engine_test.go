...
// in this test, we can set n to 1, and it means we can process Iris and commit a
// block by one node. Otherwise, if n is larger than 1, we have to generate
// other fake events to process Iris.
func newBlockChain(n int) (*core.BlockChain, *backend) {
	genesis, nodeKeys := getGenesisAndKeys(n)
	memDB := memorydb.NewMemDatabase()
...
func appendValidators(genesis *core.Genesis, addrs []common.Address) {
	...
	if len(genesis.ExtraData) < types.IrisExtraVanity {
		genesis.ExtraData = append(genesis.ExtraData, bytes.Repeat([]byte{0x00}, types.IrisExtraVanity)...)
	}
	genesis.ExtraData = genesis.ExtraData[:types.IrisExtraVanity]
	...
	if err != nil {
		panic("failed to encode iris extra")
	}
	genesis.ExtraData = append(genesis.ExtraData, istPayload...)
}

func makeHeader(parent *types.Block, config *iris.Config) *types.Header {
	header := &types.Header{
...
func TestSealStopChannel(t *testing.T) {
	...
	eventSub := engine.EventMux().Subscribe(iris.RequestEvent{})
	eventLoop := func() {
		select {
		case ev := <-eventSub.Chan():
			_, ok := ev.Data.(iris.RequestEvent)
			if !ok {
...
func TestSealCommittedOtherHash(t *testing.T) {
	...
	select {
	case <-timeout.C:
		// wait 2 seconds to ensure we cannot get any blocks from Iris
	}
}
...
func TestPrepareExtra(t *testing.T) {
	...
	vanity := make([]byte, types.IrisExtraVanity)
...
func TestWriteSeal(t *testing.T) {
	vanity := bytes.Repeat([]byte{0x00}, types.IrisExtraVanity)
	...
	expectedSeal := append([]byte{1, 2, 3}, bytes.Repeat([]byte{0x00}, types.IrisExtraSeal-3)...)
	expectedIstExtra := &types.IrisExtra{
	...
	// verify iris extra-data
	istExtra, err := types.ExtractIrisExtra(h)
	if err != nil {
		t.Errorf("error mismatch: have %v, want nil", err)
	}
...
func TestWriteCommittedSeals(t *testing.T) {
	vanity := bytes.Repeat([]byte{0x00}, types.IrisExtraVanity)
	istRawData := hexutil.MustDecode("0xf858f8549444add0ec310f115a0e603b2d7db9f067778eaf8a94294fc7e8f22b3bcdcf955dd7ff3ba2ed833f8212946beaaed781d2d2ab6350f5c4566a2c6eaac407a6948be76812f765c24641ec63dc2852b378aba2b44080c0")
	expectedCommittedSeal := append([]byte{1, 2, 3}, bytes.Repeat([]byte{0x00}, types.IrisExtraSeal-3)...)
	expectedIstExtra := &types.IrisExtra{
		Validators: []common.Address{
			common.BytesToAddress(hexutil.MustDecode("0x44add0ec310f115a0e603b2d7db9f067778eaf8a")),
			common.BytesToAddress(hexutil.MustDecode("0x294fc7e8f22b3bcdcf955dd7ff3ba2ed833f8212")),
