...
func TestCheckValidatorSignature(t *testing.T) {
	...
	for i, k := range keys {
		...
		addr, err := iris.CheckValidatorSignature(vset, data, sig)
		if err != nil {
			t.Errorf("error mismatch: have %v, want nil", err)
		}
	...
	addr, err := iris.CheckValidatorSignature(vset, data, sig)
	if err != iris.ErrUnauthorizedAddress {
		t.Errorf("error mismatch: have %v, want %v", err, iris.ErrUnauthorizedAddress)
	}
...
func TestCommit(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			// normal case
			nil,
			[][]byte{append([]byte{1}, bytes.Repeat([]byte{0x00}, types.IrisExtraSeal-1)...)},
			func() *types.Block {
...
func TestCalConsensusTimeRatio(t *testing.T) {
	...
	go func() {
		sb.EventMux().Post(irisCore.TimeoutEvent{})
...
func newTestValidatorSet(n int) (iris.ValidatorSet, []*ecdsa.PrivateKey) {
	// generate validators
	keys := make(Keys, n)
