...
var (
	...
	errDecodeFailed = errors.New("fail to decode iris message")
)
...
func (sb *backend) decode(msg p2p.Msg) ([]byte, common.Hash, error) {
	...
	return data, iris.RLPHash(data), nil
}
...
func (sb *backend) HandleMsg(addr common.Address, msg p2p.Msg) (bool, error) {
	...
	if msg.Code == istanbulMsg {
		if !sb.coreStarted {
			return true, iris.ErrStoppedEngine
		}
		...
		go sb.msgFeed.Send(iris.MessageEvent{
			Payload: data,
		})
	...
	if msg.Code == NewBlockMsg && sb.core.IsProposer() { // eth.NewBlockMsg: import cycle
		...
		if reader, ok := msg.Payload.(*bytes.Reader); ok {
			...
			if newRequestedBlock.Header().MixDigest == types.IrisDigest && sb.core.IsCurrentProposal(newRequestedBlock.Hash()) {
...
func (sb *backend) NewChainHead() error {
	...
	go sb.istanbulEventMux.Post(iris.FinalCommittedEvent{})
	return nil
}
