...
func newTestPreprepare(v *iris.View) *iris.Preprepare {
	return &iris.Preprepare{
		View:     v,
...
func TestHandlePreprepareWithLock(t *testing.T) {
	...
	testCases := []struct {
		system       *testSystem
		proposal     iris.Proposal
		lockProposal iris.Proposal
	}{
		{
			newSystem(),
			proposal,
	...
	for _, test := range testCases {
		...
		lockPreprepare := &iris.Preprepare{
			View:     curView,
