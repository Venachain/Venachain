...
func newRoundState(view *iris.View, validatorSet iris.ValidatorSet, lockedHash common.Hash,
	preprepare *iris.Preprepare, pendingRequest *iris.Request,
...
type roundState struct {
	round          *big.Int
	sequence       *big.Int
	Preprepare     *iris.Preprepare
	Prepares       *messageSet
	Commits        *messageSet
	lockedHash     common.Hash
	pendingRequest *iris.Request
...
func (s *roundState) Subject() *iris.Subject {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *roundState) SetPreprepare(preprepare *iris.Preprepare) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *roundState) Proposal() iris.Proposal {
	s.mu.RLock()
	defer s.mu.RUnlock()
