...
func (self *testSystemBackend) Validators(proposal iris.Proposal) iris.ValidatorSet {
	return self.peers
}
...
func (self *testSystemBackend) Send(message []byte, target common.Address) error {
	...
	self.sys.queuedMessage <- iris.MessageEvent{
		Payload: message,
	}
	return nil
}

func (self *testSystemBackend) Broadcast(valSet iris.ValidatorSet, message []byte) error {
	testLogger.Info("enqueuing a message...", "address", self.Address())
	self.sentMsgs = append(self.sentMsgs, message)
	self.sys.queuedMessage <- iris.MessageEvent{
		Payload: message,
...
func (self *testSystemBackend) Gossip(valSet iris.ValidatorSet, message []byte) error {
	testLogger.Warn("not sign any data")
	return nil
}

func (self *testSystemBackend) Commit(proposal iris.Proposal, seals [][]byte) error {
	...
	go self.events.Post(iris.FinalCommittedEvent{})
	return nil
}

func (self *testSystemBackend) Verify(proposal iris.Proposal) (time.Duration, error) {
	return 0, nil
}
...
func (self *testSystemBackend) NewRequest(request iris.Proposal) {
	go self.events.Post(iris.RequestEvent{
		Proposal: request,
	})
}
...
func (self *testSystemBackend) LastProposal() (iris.Proposal, common.Address) {
...
func (self *testSystemBackend) ParentValidators(proposal iris.Proposal) iris.ValidatorSet {
	return self.peers
}
...
type testSystem struct {
	...
	queuedMessage chan iris.MessageEvent
	quit          chan struct{}
}
...
func newTestSystem(n uint64) *testSystem {
	...
	return &testSystem{
		...
		queuedMessage: make(chan iris.MessageEvent),
...
func newTestValidatorSet(n int) iris.ValidatorSet {
	return validator.NewSet(generateValidators(n), iris.RoundRobin)
}
...
func NewTestSystemWithBackend(n, f uint64) *testSystem {
	...
	for i := uint64(0); i < n; i++ {
		vset := validator.NewSet(addrs, iris.RoundRobin)
		...
		core.current = newRoundState(&iris.View{
			Round:    big.NewInt(0),
