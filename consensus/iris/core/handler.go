...
func (c *core) subscribeEvents() {
	...
	c.finalCommittedSub = c.backend.EventMux().Subscribe(
		iris.FinalCommittedEvent{},
	)

	c.msgCh = make(chan iris.MessageEvent)
...
func (c *core) unsubscribeEvents() {
	...
	event, err := c.backend.GetConsensusTypeMuxSub()
	if err != nil {
		c.logger.Error("unsubscribeEvents", "GetConsensusTypeMuxSub", err)
	}
	event.Unsubscribe()
}
...
func (c *core) handleEvents() {
	...
	for {
		...
		case event, ok := <-c.events.Chan():
			...
			case iris.SingleCommittedEvent:
...
func (c *core) handleMsg(payload []byte) error {
	...
	if err := msg.FromPayload(payload, c.validateFn); err != nil {
		...
		logger.Info("Failed to decode message from payload", "notice", err)
		return err
	}
	...
	if src == nil {
		...
		return iris.ErrUnauthorizedAddress
	}

	return c.handleCheckedMsg(msg, src)
}

func (c *core) handleCheckedMsg(msg *message, src iris.Validator) error {
