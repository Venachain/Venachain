...
type defaultSet struct {
	validators iris.Validators
	policy     params.ProposerPolicy

	proposer    iris.Validator
	validatorMu sync.RWMutex
	selector    iris.ProposalSelector
}
...
func newDefaultSet(addrs []common.Address, policy params.ProposerPolicy) *defaultSet {
	...
	// init validators
	valSet.validators = make([]iris.Validator, len(addrs))
	...
	if params.ProposerPolicy(policy) == iris.Sticky {
...
func (valSet *defaultSet) List() []iris.Validator {
	valSet.validatorMu.RLock()
	defer valSet.validatorMu.RUnlock()
	return valSet.validators
}

func (valSet *defaultSet) GetByIndex(i uint64) iris.Validator {
	valSet.validatorMu.RLock()
	defer valSet.validatorMu.RUnlock()
...
func (valSet *defaultSet) GetByAddress(addr common.Address) (int, iris.Validator) {
...
func (valSet *defaultSet) GetProposer() iris.Validator {
...
func calcSeed(valSet iris.ValidatorSet, proposer common.Address, round uint64) uint64 {
	offset := 0
...
func roundRobinProposer(valSet iris.ValidatorSet, proposer common.Address, round uint64) iris.Validator {
	if valSet.Size() == 0 {
		return nil
	}
	seed := uint64(0)
...
func stickyProposer(valSet iris.ValidatorSet, proposer common.Address, round uint64) iris.Validator {
	if valSet.Size() == 0 {
		return nil
	}
	seed := uint64(0)
...
func (valSet *defaultSet) Copy() iris.ValidatorSet {
	valSet.validatorMu.RLock()
	defer valSet.validatorMu.RUnlock()
