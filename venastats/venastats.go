...
type Service struct {
	...
	eth    *vena.Ethereum     // Full Ethereum service if monitoring a full node
...
func New(url string, ethServ *vena.Ethereum, lesServ *les.LightEthereum) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["eth"]; info != nil {
		network = fmt.Sprintf("%d", info.(*vena.NodeInfo).Network)
		protocol = fmt.Sprintf("eth/%d", vena.ProtocolVersions[0])
	} else {
